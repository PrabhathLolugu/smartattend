...
const sql = `
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS role_type TEXT DEFAULT 'student';
...
BEGIN
  ...
  student_stats AS (
    SELECT 
      ...
      st.roll_number,
      st.name,
      COALESCE(st.role_type, 'student') AS role_type,
      st.department,
      st.program,
      st.group_label,
...
try {
  ...
  console.log('Added role_type column and updated student_attendance_summary successfully!');
} catch (err) {
